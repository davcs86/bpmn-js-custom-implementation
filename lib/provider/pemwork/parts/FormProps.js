...
//});
function formFieldCheckboxField(options, getSelectedFormField) {
...
module.exports = function(group, element, bpmnFactory, elementRegistry, translate) {
  ...
  {   var root = elementRegistry.getRootElement();
    ...
    eventbus('element.create', function(){

      var commands = [];

      commands.push(cmdHelper.updateBusinessObject(element, bo, { 'pemwork:FormData': undefined }));
  ...
  function getSelectedFormField2(element, node) {
    var root = elementRegistry.getAll()[0];
    var selected = formFieldsEntry.getSelected(root, node.parentNode);

    if (selected.idx === -1) {
      return;
    }

    return formHelper.getFormField(root, selected.idx);
  }
  ...
  group.entries.push({
    ...
    set: function(element, values, node) {
      ...
      //} else {
        ...
        var formData = elementHelper.createElement('pemwork:FormData', { fields: [] }, extensionElements, bpmnFactory);
  ...
  var formFieldsEntry = extensionElements(element, bpmnFactory, {
    ...
    createExtensionElement: function(element, extensionElements, value){
      ...
      var bo = getBusinessObject(root),
          ...
          field = elementHelper.createElement('pemwork:FormField', { id: value }, formData, bpmnFactory),
          commands = [];
      ...
      if (isUndefined(formData)){
        ...
        formData = elementHelper.createElement('pemwork:FormData',
    ...
    removeExtensionElement: function(element, extensionElements, value, idx) {
     /* var root = elementRegistry.getAll()[0];
      ...
      var formData = getExtensionElements(getBusinessObject(element), 'pemwork:FormData')[0],
          entry = formData.fields[idx];
...
//console.log(translate('Form Field'));
  ...
  group.entries.push(formFieldCheckboxField({
    id : 'form-field-requiredValue',
    label : 'Required',
    modelProperty: 'required'
  }, getSelectedFormField2));
  group.entries.push(formFieldCheckboxField({
    id : 'form-field-readonlyValue',
    label : 'ReadOnly',
    modelProperty: 'readonly'
  }, getSelectedFormField2));
};