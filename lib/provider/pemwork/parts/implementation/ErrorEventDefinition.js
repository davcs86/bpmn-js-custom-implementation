...
function ErrorEventDefinition(group, element, bpmnFactory, errorEventDefinition, showErrorCodeVariable) {
  group.entries.push({
    ...
    'html': '<div class="pp-row pp-select">' +
              '<label for="pemwork-errors">Errors</label>' +
              '<div class="pp-field-wrapper">' +
                '<select id="pemwork-errors" name="errors" data-value>' +
                '</select>' +
  ...
  if (showErrorCodeVariable) {
    group.entries.push(entryFactory.textField({
      ...
      get: function(element) {
        ...
        var boErrorCodeVariable = errorEventDefinition.get('pemwork:errorCodeVariable');
      ...
      set: function(element, values) {
        var update = {
          'pemwork:errorCodeVariable' : undefined
        };

        if (values.errorCodeVariable !== '') {
          update['pemwork:errorCodeVariable'] = values.errorCodeVariable;
        }
