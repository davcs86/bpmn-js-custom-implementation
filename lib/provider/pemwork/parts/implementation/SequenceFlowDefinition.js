...
function TimerEventDefinition(group, element, bpmnFactory, timerEventDefinition, translate) {
  group.entries.push({
    ...
    'description': translate('Configure the timer event definition'),
    ...
    '  <option value="64">June</option>'+
    '  <option value="128">July</option>'+
    '  <option value="256">August</option>'+
    '  <option value="512">September</option>'+
    ...
    '  <option value="16">Fourth</option>'+
    '  <option value="32">Last</option>'+
    '</select>'+
    '</div>' +
    ...
    '  <option value="32">Thursday</option>'+
    '  <option value="64">Friday</option>'+
    '  <option value="128">Saturday</option>'+
    '</select>'+
    '</div> <br/>' +
    ...
    get: function (element, propertyNode) {
      ...
      setTimeout(function(){
        ...
        if (([4, 5]).indexOf(timerEventDefinitionObj.tab) >= 0 && timerEventDefinitionTypeValue === 'timeCycle') {

          console.log(timerEventDefinitionObj.tab);
          // get the days
          var selectTimerCycleWeekdaysDOM = domQuery('select[name=timerCycleWeekdays]', propertyNode);
          // selected days
          var selectedWeekdays = timerEventDefinitionObj.interval[2];
          forEach([256, 128, 64, 32, 8, 4, 2], function(item){
            if (selectedWeekdays>=item){
              var option = domQuery('option[value=\''+item+'\']', selectTimerCycleWeekdaysDOM);
              domAttr(option, 'checked', 'checked');
              selectedWeekdays -= item;
            }
          });
          if (timerEventDefinitionObj.tab == '5') {
            // get the months
            var selectTimerCycleMonthsDOM = domQuery('select[name=timerCycleMonths]', propertyNode);
            // selected months
            var selectTimerCycleMonths = timerEventDefinitionObj.interval[0];
            forEach([4096, 2048, 1024, 512, 256, 128, 64, 32, 16, 8, 4, 2], function(item){
              if (selectTimerCycleMonths>=item){
                var option = domQuery('option[value=\''+item+'\']', selectTimerCycleMonthsDOM);
                domAttr(option, 'checked', 'checked');
                selectTimerCycleMonths -= item;
              }
            });
            // get the weeks
            var selectTimerCycleWeeksDOM = domQuery('select[name=timerCycleWeeks]', propertyNode);
            // selected weeks
            var selectTimerCycleWeeks = timerEventDefinitionObj.interval[1];
            forEach([32, 16, 8, 4, 2], function(item){
              if (selectTimerCycleWeeks>=item){
                var option = domQuery('option[value=\''+item+'\']', selectTimerCycleWeeksDOM);
                domAttr(option, 'checked', 'checked');
                selectTimerCycleWeeks -= item;
              }
            });
          }
        }
