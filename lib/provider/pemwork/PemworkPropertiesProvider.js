...
var nameProps = require('./parts/NameProps'),
    ...
    ScriptTaskProps = require('./parts/ScriptTaskProps'),
    SequenceFlowsProps = require('./parts/SequenceFlowsProps');
...
function createGeneralTabGroups(element, bpmnFactory, elementRegistry, translate, configSettings) {
 ...
 // SequenceFlowProps(generalGroup, element, bpmnFactory, translate);
  ...
  ScriptTaskProps(generalGroup, element, translate, configSettings);
 /* var flowGroup = {
    id: 'flows',
    label: translate('flow conditions'),
    entries: []
  };*/
  //SequenceFlowsProps(flowGroup, element,bpmnFactory, elementRegistry, translate);
  return[
    generalGroup,
    timersGroup//,flowGroup
  ];
}
function createFlowSequenceTabGroups(element, bpmnFactory, elementRegistry, translate, configSettings) {
...
function createFormsTabGroups1(element, bpmnFactory, elementRegistry, translate, configSettings) {
 ...
 FormProps(FormsGroups, element, bpmnFactory, elementRegistry, translate);
...
function PemworkPropertiesProvider(eventBus, bpmnFactory, elementRegistry, translate, configSettings) {
  ...
  this.getTabs = function(element) {
    ...
    var flowConditionTab = {
      id: 'flowCondition',
      label:translate('Condition'),// 'Forms',
      groups:createFlowSequenceTabGroups(element, bpmnFactory, elementRegistry, translate, configSettings)
    };
//var tabPrueba={
   // id:'prueba',
  //  label:'prueba',
